...
public class AdminDaoImpl implements AdminDao {
    ...
    public boolean signUp(User userInput ,String otpInput) {
        ...
        try {
         ...
         // Make sure OTP entity is managed (attached to the session)
            EmailVerificationOtp managedOtp = (EmailVerificationOtp) session.get(
                EmailVerificationOtp.class, usedOtp.getOtpId()
            );

            if (managedOtp != null) {
                managedOtp.setStatus(OtpStatus.EXPIRED);
                managedOtp.setExpiresAt(new Timestamp(System.currentTimeMillis()));
                // No need to call session.update(managedOtp); it's managed
            }
